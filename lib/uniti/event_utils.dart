...
import 'package:flutter_app/model/PushEventCommit.dart';
...
class EventUtils {
  ///事件描述与动作
  static getActionAndDes(evtent event) {
    String actionStr;
    String des;
    switch (event.type) {
      ...
      case "CreateEvent":
        if (event.payload.refType == "repository") {
          ...
          actionStr =
              "Created " + event.payload.refType + " " + event.payload.ref +
                  " at " + event.repo.name;
        }
        break;
      case "DeleteEvent":
        actionStr =
            "Delete " + event.payload.refType + " " + event.payload.ref +
                " at " + event.repo.name;
        break;
      case "ForkEvent":
      ...
      case "IssueCommentEvent":
        actionStr = event.payload.action + " comment on issue " +
            event.payload.issue.number.toString() + " in " + event.repo.name;
      ...
      case "ReleaseEvent":
        actionStr =
            event.payload.action + " release " + event.payload.release.tagName +
                " at " + event.repo.name;
        break;
      case "WatchEvent":
